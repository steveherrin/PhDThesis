...
\begin{subfigure}[b]{0.5\linewidth}
\centering
\includegraphics[width=1.0\columnwidth]{errm_fit.eps}
\end{subfigure}%
\begin{subfigure}[b]{0.5\linewidth}
\centering
\includegraphics[width=1.0\columnwidth]{errp_fit.eps}
\end{subfigure}
\caption[Fits to errors on electron lifetime measurements]{The width of the uncertainty bars on single electron lifetime measurements as a function of the electron lifetime. Measurements from TPC 1 are used because they are observed to have larger uncertainties. Since the uncertainties  are asymmetric, the negative uncertainty (left) and the positive uncertainty (right) are fit separately. The fit function is a quadratic polynomial. ROOT's robust linear fitting does not return errors on fit parameters.}
\label{fig:err_fits}
\end{figure}

As shown in \cref{fig:err_fits}\todo{Fix the plot to use the widths above}, a quadratic function nicely fits the observed uncertainties. So, at first glance, it appears as though the resolution should be independent of electron lifetime, due to the \(1/\tau^2\) term in \cref{eq:de_dtau}. However, because there are linear terms, and because the uncertainties are not symmetric around the measurement, this is not the case. I tabulate the effect on electron lifetime in \cref{tab:res_dtau}
...
The numbers shown in \cref{tab:res_dtau} \todo{Update this table.} reflect using only a single electron lifetime measurement. As described below, a number of measurements go into the actual correction applied to data.
