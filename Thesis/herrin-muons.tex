...
\frac{dN}{d\Omega} \propto \left (cos \theta \right)^{1.53}e^{\left(-8\times10^{-4} h \left(sec \theta -1\right)\right)}
\end{equation}
...
\frac{dN}{dE} \propto e^{\left(-8\times10^{-4} h\cdot 2.77 \right)}\left(E+693\left(1-e^{-8\times10^{-4} h}\right)\right)^{-3.77}
\end{equation}
