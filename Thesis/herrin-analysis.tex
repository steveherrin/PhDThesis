...
While the rate agreement with calibration sources provides a conservative estimate of the accuracy to which the detector can measure decay rates, it is possible to estimate this uncertainty directly. \Cref{tab:analysis_normalization_uncertainty} summarizes these estimates.
...
Falsely finding a second scintillation signal			&	0.5	&	0.5	&	0.5	&	0.5	&	1.0	\\
...
%\textbf{Total}									&\textbf{4.89}&\textbf{4.91}&\textbf{4.89}&\textbf{4.89}&\textbf{5.10}\\\bottomrule
...
%Noise fluctuations in the APD waveforms might be reconstructed as scintillation signals. These events would be rejected by the requirement that events have only one scintillation signal. About \SI{0.75}{\percent} of events in real data are removed by this cut, while \SI{4}{\percent} of events in Monte Carlo simulations of \twonu{} events are removed. Since events in the data can be attributed to genuine coincidences (involving \isotope{214}{Bi}), no such events are in the \twonu{} simulations. Therefore, this \SI{4}{\percent} is taken as a systematic error.
Noise fluctuations in the APD waveforms might be reconstructed as scintillation signals. These events would be rejected by the requirement that events have only one scintillation signal. To study this, simulated signals were added to pulsed trigger events taken from physics data. The rate for falsely reconstructing a second scintillation signal was \SI{1}{\percent} at worst, when the simulated signal was below about \SI{700}{\keV}. For simulated signals about \SI{1500}{\keV} and above, the rate was \SI{0.1}{\percent}. Therefore, integrated over the energy spectra of the modes, the systematic error is less than \SI{0.5}{\percent} for all modes expect the one with spectral index 7, which is less than \SI{1}{\percent}.
...
\begin{table}[htp]
\centering
\caption[Effect of uncertainties on the half life measurement]{A breakdown of the various uncertainties on the measurement of the \twonu{} half life. Each contribution is given as a percentage of the best-fit value}
...
\zeronuX{}	&	1	&	\(>2.6\times10^{24}\)		&	\(>6.34\times10^{23}\)		\\
\zeronuX{}	&	2	&	\(>1.0\times10^{24}\)		&	\(>2.31\times10^{23}\)		\\
\zeronuXpX{}	&	3	&	\(>4.5\times10^{23}\)		&	\(>7.01\times10^{22}\)		\\
\zeronuXX{}	&	7	&	\(>1.1\times10^{22}\)		&	\(>1.30\times10^{22}\)		\\\bottomrule
\end{tabular}
\end{table}

\subsection{Effect of Systematic Uncertainties}
\label{sec:analysis_majoron_error_budget}
\Cref{tab:analysis_majoron_error_budget} tabulates the effect of the various uncertainties on the limits obtained for the Majoron-emitting modes. All of the limits are only slightly degraded below their statistical limits. For all modes, uncertainty in the backgrounds has a modest effect on the limit. Being able to better constrain the backgrounds, both through external constraints and more detector data, could improve the limits. For the modes with spectral indices 1, 2, and 3, the statistical limits are weakened by uncertainties in the background and the systematic uncertainty in the single site fraction. Since these events are similar to \twonu{} events, this suggests the limits could be improved modestly by using the \twonu{} single site fraction as a constraint on the single site fraction for these modes. The mode with spectral index 7 has its limit weakened by the normalization uncertainty, and this seems to be correlated with the background uncertainty, since the overall limit is no worse. Looking at the energy spectrum, this mode is similar in shape to \twonu{}, and so changes in its amplitude can be compensated for in similar ways by either changing the normalization or the number of \twonu{}.
...
Uncertainty		&	\multicolumn{4}{c}{90\% CL limit on \(T_{1/2}\)}			\\
				&	\(n = 1\) (\SI{e23}{\year})		&	\(n = 2\) (\SI{e23}{\year})		&	\(n = 3\) (\SI{e22}{\year})		&	\(n=4\) (\SI{e22}{\year})	\\\midrule
Statistical only		&	6.49						&	2.36						&	6.87						&	1.34					\\
SS fraction	&	6.42						&	2.33						&	6.76						&	1.34						\\
Backgrounds	&	6.43						&	2.34						&	6.79						&	1.30						\\
Normalization	&	6.49						&	2.36						&	6.87						&	1.30						\\\midrule
\textbf{Reported}	&	\textbf{6.34}				&	\textbf{2.31}				&	\textbf{6.67}				&	\textbf{1.30}			\\\bottomrule
\end{tabular}
\end{table}
...
Using the best fit spectrum with no \zeronuXpX{} to again create toy simulations, it is possible to study the sensitivity to Majoron-emitting modes. Each simulation is used to construct a 90\% confidence interval. \Cref{fig:analysis_bb0nX_sensitivity} shows histograms of the limits obtained from many simulations. For all modes, the expected sensitivity is higher than the observed limit. For the modes with spectral indices 1 and 2, higher limits are expected 77\% and 81\% of the time, respectively. For the mode with spectral index 3, 98\% of simulations had a better limit. For the mode with spectral index 7, only one simulation in 500 had a comparable limit. This suggests that the model used to simulate the data is inadequate. As discussed in \cref{sec:analysis_shape_agreement} the simulations show a slightly different single-site fraction than seen in data. Since the \zeronuXX{} spectrum is primarily single site and peaks at lower energies than \twonu{}, the upper limit on the number of decays is larger in data than in the toy simulations.
