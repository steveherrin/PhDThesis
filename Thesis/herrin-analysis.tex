...
One simple check is to compare the shapes of the energy and standoff distance distributions in data and simulation. \Cref{fig:analysis_shape_agreement} shows this comparison. The overall agreement in both shape and standoff distance looks reasonable. However, looking at the residual plots shown in \cref{fig:analysis_shape_agreement_residuals} shows that the agreement is not perfect.
...
\centering
	\begin{subfigure}[c]{0.48\textwidth}
	\centering
	\includegraphics[width=\textwidth]{./plots/analysis_shape_agreement_E_residual.pdf}
	\end{subfigure}\hfill%
	\begin{subfigure}[c]{0.48\textwidth}
	\centering
	\includegraphics[width=\textwidth]{./plots/analysis_shape_agreement_sd_residual.pdf}
	\end{subfigure}
\caption[Residuals for data minus simulation for a \isotope{228}{Th} calibration source]{The residuals for data minus simulation for the energy (left) and standoff distance distributions (right) of a \isotope{228}{Th} source located at the cathode. This is for single site events. The \(y\)-axis is the number of standard deviations the data is from the simulation.}
\label{fig:analysis_shape_agreement_residuals}
\end{figure}
