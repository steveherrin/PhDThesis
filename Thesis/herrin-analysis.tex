...
Noise fluctuations in the APD waveforms might be reconstructed as scintillation signals. These events would be rejected by the requirement that events have only one scintillation signal. About \SI{0.75}{\percent} of events in real data are removed by this cut, while \about{}\SI{4}{\percent} of events in Monte Carlo simulations of \twonu{} events (which constitute most events in real data) are removed. This is attributed to the matched filter signal finding technique being optimized to work on APD signals with real noise, when the Monte Carlo simulations only have simulated white noise. Events in the data can be attributed to genuine coincidences (involving \isotope{214}{Bi}). No genuine coincidences are in the \twonu{} or \zeronuXpX{} simulations. For each double beta decay mode, the rate of events that have false coincidences is determined. When computing the efficiency for double beta events to survive all cuts, all of these false coincidences could survive the other cuts, or all could fail. Therefore, the efficiency is taken midway between these extremes, and half the false coincident rate is propagated as a systematic error.
