...
Falsely finding of second scintillation signal			&	\multicolumn{5}{c}{\(1.0\)}				\\
...
Clustering efficiency								&	\multicolumn{5}{c}{\(1.0\)}	\\
Shape distortion in \SIrange{700}{1000}{\keV}			&	\multicolumn{5}{c}{0.75}					\\\midrule
\textbf{Total}									&\textbf{3.0}&\textbf{3.0}&\textbf{3.0}&\textbf{3.0}&\textbf{3.3}\\\bottomrule
\end{tabular}
\end{table}
...
Noise fluctuations in the APD waveforms might be reconstructed as scintillation signals. These events would be rejected by the requirement that events have only one scintillation signal. In order to estimate this rate, a study was done in which simulated pulses were added to the waveforms from the \SI{0.1}{\Hz} pulsed triggers. These events had no real signals in them. The rate of falsely reconstructing a second signal (in addition to the injected one) was seen to be at worst \SI{1}{\percent}.
