...
def Collapse(h):
    h_1d = ROOT.TH1D(h.GetName()+"_bins", h.GetTitle() + " Bin Distribution",
                     100, 0, 1e-6)
    for i in xrange(h.fN):
        if (h.IsBinOverflow(i) or h.IsBinUnderflow(i)):
            continue
        if h.GetBinContent(i) > 0:
            h_1d.Fill(h.GetBinContent(i))
...
def ProjectedAreaSin(theta, phi):
    # with a sin(theta) factor for integration
    return ProjectedArea(theta, phi)*sin(theta)
...
h_rec = ROOT.TH2D("h_rec","Number Reconstructed",
                  n_p, p_lo, p_hi, n_t, 0, max_theta)

h_obs = ROOT.TH2D("h_obs","Number Observed",
                  n_p, p_lo, p_hi, n_t, 0, max_theta)
...
h_I = ROOT.TH1D("h_I", "I(h, 0)", 50, 0, 1e-6)
h_I_w = ROOT.TH1D("h_I_w", "I(h, 0) Weighted", 50, 0, 1e-6)
h_flux_sc = ROOT.TH1D("h_flux_sc", "Scaled Flux", 50, 0, 1e-6)
h_vflux_sc = ROOT.TH1D("h_vflux_sc", "Scaled Vertical Flux", 50, 0, 1e-6)
g_I = ROOT.TGraphErrors()
g_I.SetNameTitle("g_I","I(h, 0);bin number;I(h, 0) (Hz cm^{-1} sr^{-1})")
...
g_good_region_pos = ROOT.TGraph(len(good_phi) + 1)
g_good_region_neg = ROOT.TGraph(len(good_phi) + 1)
...
g_good_region_pos.SetPoint(len(good_phi), pi, good_theta[-1])
g_good_region_neg.SetPoint(len(good_phi), -pi, good_theta[-1])
...
for i in xrange(h_flux.fN):
    x = ROOT.Long(0)
    y = ROOT.Long(0)
    z = ROOT.Long(0)
    h_flux.GetBinXYZ(i, x, y, z)

    theta_min = h_flux.GetYaxis().GetBinLowEdge(y)
    theta_max = h_flux.GetYaxis().GetBinUpEdge(y)
    phi_min = h_flux.GetXaxis().GetBinLowEdge(x)
    phi_max = h_flux.GetXaxis().GetBinUpEdge(x)
    
    
    if theta_min == 0:
        theta = 0
        #first_bin_theta = bin.GetYMax()
        #h_flux.SetBinContent(i, 0)
        #h_vflux.SetBinContent(i, 0)
        #continue
    else:
        #theta = (bin.GetYMin()+bin.GetYMax())/2
        theta = (theta_min + theta_max)/2
        #theta = (scipy.integrate.quad(lambda x: x*sin(x), theta_min,
        #                              theta_max)[0]/
        #         scipy.integrate.quad(lambda x: sin(x), theta_min,
        #                              theta_max)[0])

    phi = (phi_max + phi_min)/2

    #h_check.SetBinContent(i, g_good_region.IsInside(phi_sym, theta))
    
        

    if (g_good_region.IsInside(phi, theta) == 1 or
        g_good_region_pos.IsInside(phi, theta) == 1 or
        g_good_region_neg.IsInside(phi, theta) == 1):

        #print ("(t, p) = (%0.2f, %0.2f)"%(theta, phi))
        #print ("   phi = %0.2f - %0.2f"%(phi_min, phi_max))
        #print (" theta = %0.2f - %0.2f"%(theta_min, theta_max))
            

        # solid angle of the bin
        d_phi = phi_max - phi_min
        d_theta = theta_max - theta_min
        sa = d_phi*(cos(theta_min)-cos(theta_max))
        sum_sa += sa
    
        N = h_num.GetBinContent(i)
        h_check.SetBinContent(i, N)
        N_sim += h_sim.GetBinContent(i)
        N_rec += h_rec.GetBinContent(i)
        eff = h_eff.GetBinContent(i)
        N_total += N
    
        Aint = scipy.integrate.dblquad(ProjectedAreaSin,
                                    phi_min, phi_max,
                                    lambda x: theta_min,
                                    lambda x: theta_max)[0]/sa
        A = ProjectedArea(theta, phi)
        print("A check: %0.2e - %0.2e = %0.2e"%(A, Aint, fabs(A-Aint)))

        try:
            flux_and_error = GetFluxAndError(N,  h_rec.GetBinContent(i), h_sim.GetBinContent(i), A*sa*dT)
            h_flux.SetBinContent(i, flux_and_error[0])
            ...
            h_I.Fill(flux_and_error[0]/flux(theta, 1585))
            h_I_w.Fill(flux_and_error[0]/flux(theta, 1585), flux_and_error[0]/flux_and_error[1])
            n = g_I.GetN()
            g_I.Set(n+1)
            g_I.SetPoint(n, i, flux_and_error[0]/flux(theta, 1585))
            g_I.SetPointError(n, 0, flux_and_error[1]/flux(theta, 1585))

            ratio_flux = (d_phi*scipy.integrate.quad(flux_sin, theta_min,
                                                     theta_max, (1585))[0]
                          /(2*pi*scipy.integrate.quad(flux_sin, 0, pi/2,
                                                      (1585))[0]))
            h_flux_sc.Fill(flux_and_error[0]/ratio_flux*sa/(2*pi))

            ratio_vflux = (d_phi*scipy.integrate.quad(flux_sin, theta_min,
                                                   theta_max, (1585))[0]
                          /(2*pi*scipy.integrate.quad(vertical_flux_sin,
                                                      0, pi/2,
                                                      (1585))[0]))
            h_vflux_sc.Fill(flux_and_error[0]/ratio_vflux*sa/(2*pi))

            #if (i % 10 == 0):
            #    print("unscaled: %0.2e, flux: %0.2e, vflux: %0.2e, ratio: %0.2e, vratio: %0.2e"%(flux_and_error[0], flux_and_error[0]/ratio_flux, flux_and_error[0]/ratio_vflux, ratio_flux, ratio_vflux))
            ...
            flux_and_error = GetFluxAndError(N,  h_rec.GetBinContent(i), h_sim.GetBinContent(i), A*sa*dT*cos(theta))
            h_vflux.SetBinContent(i, flux_and_error[0])
            #n_g = g_vflux.GetN()
            #g_vflux.Set(n_g+1)
            ...
            print("N_rec = %f, N_sim = %f, A = %f"%(h_rec.GetBinContent(i), h_sim.GetBinContent(i), A))
...
total_flux = flux_in_region / 4.221e-01
total_flux_err = flux_in_region_err / 4.221e-01
total_flux_errm = total_flux - flux_in_region / 4.240e-01
total_flux_errp = flux_in_region / 4.200e-01 - total_flux

total_vflux = flux_in_region / 5.170e-01
total_vflux_err = flux_in_region_err / 5.170e-01
total_vflux_errm = total_vflux - flux_in_region / 5.211e-01
total_vflux_errp = flux_in_region / 5.128e-01 - total_vflux
...
c_I_w = ROOT.TCanvas("c_I_w")
h_I_w.Draw("")

c_I_g = ROOT.TCanvas("c_I_g")
g_I.SetMarkerStyle(ROOT.kFullCircle)
g_I.Draw("AP")
...
c_flux_sc = ROOT.TCanvas("c_flux_sc")
h_flux_sc.Draw("")

c_vflux_sc = ROOT.TCanvas("c_vflux_sc")
h_vflux_sc.Draw("")
